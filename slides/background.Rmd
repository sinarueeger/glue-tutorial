...
- works sometimes on servers, hence may not be able to install glue there
...
```{r}
library(glue)
library(dplyr) ## contains the starwars dataset
```
...
## Solution
2, the R code creates `pine,fir,cedar,spruceandhemlock`. 
...
dat <- data.frame(element = c(1, 1, 2, 2), measure = c("A", "B", "A", "B"))
dat
```

create an extra column `id_measure` using `glue`:
